...
# Asumiendo que df_items_unicos tiene las columnas 'item_id', 'item_name' y las columnas de géneros
...
def recommend_games(item_id: int = Query(..., title="ID del juego")) -> List[str]:
    ...
    try:
        ...
        genre_cols = ['Action', 'Adventure', 'Indie', 'RPG']
        genre = max(genre_cols, key=lambda col: df_items_unicos.loc[item_id, col])
        ...
        juegos_recomendados = df_items_unicos.index[fila_similitud.argsort()[::-1][:5]]
        
        # Filtrar los juegos recomendados por el género asociado al 'item_id'
        juegos_recomendados = [juego for juego in juegos_recomendados if df_items_unicos.loc[juego, genre] == 1]
